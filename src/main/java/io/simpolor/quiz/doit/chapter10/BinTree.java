...
public class BinTree<K, V> {
    ...
    private void addNode(Node<K,V> node, K key, V data){
        ...
        if(cond == 0){
            ...
            if(node.left == null){ // key가 이진 검색트리에 이미 있음
    ...
    public boolean remove(K key){
        ...
        boolean isLeftChild = true; // p는 부모의 왼쪽 자식의 노드인가?

        while (true){
            if(p == null){ // 더 이상 진행하지 않으면
                return false; // 그 키 값은 없습니다.
            }
            int cond = comp(key, p.getKey()); // key와 노드의 p의 키 값을 비교
            if(cond == 0){ // 같으면
                break; // 검색 성공
            }else{
        ...
        if(p.left == null){ // p에는 왼쪽 저식이 없음
            ...
            if(p == root){
                ...
                parent.left = p.left; // 부모의 왼쪽 포인털가 왼쪽 자식을 기리킴
            }else{
                parent.right = p.left; // 부모의 오른쪽 포이너가 왼쪽 자식을 가리킴
            }
        }else{
            parent = p;
    ...
    private void printSubTree(Node node){
        if(node != null){
            ...
            printSubTree(node.right); // 오른쪽 서브 트리를 키 값의 오른차순으로 출력
        }
    }
